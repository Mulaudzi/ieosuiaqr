...
class MailService
{
    ...
    private static function getMailer(): PHPMailer
    {
        if (self::$mailer === null) {
            ...
            self::$mailer->SMTPAuth = filter_var(
                $_ENV['SMTP_AUTH'] ?? 'true',
            ...
            self::$mailer->SMTPSecure = $smtpSecure === 'none' ? '' : $smtpSecure;
            self::$mailer->SMTPAutoTLS = filter_var(
                $_ENV['SMTP_AUTO_TLS'] ?? 'true',
                FILTER_VALIDATE_BOOLEAN
            );
            ...
            // SMTP protocol output must never be written into an HTTP response.
            // Enable it independently when diagnosing mail and send it to logs.
            self::$mailer->SMTPDebug = filter_var(
                $_ENV['SMTP_DEBUG'] ?? 'false',
                FILTER_VALIDATE_BOOLEAN
    ...
    public static function send(string $to, string $subject, string $htmlBody, ?string $textBody = null): bool
    {
        try {
            ...
            // Recipient
            $mail->addAddress($to);
            $mail->addReplyTo($_ENV['SUPPORT_EMAIL'] ?? 'support@ieosuia.com', 'IEOSUIA Support');
